...
## Installation Using PowerShell (Version 0.4 - Released on 2023-10-12)

To deploy SQL Exporter on Windows configured for SQL Server, follow these steps:
1. Edit the deployment folder path by updating the `$DeployFolder` variable in the script located at:
   `c:\Git\SQL-Server-admin\Prometheus\AP\deploy.ps1`.
2. Run the PowerShell script as an Administrator. This script will:
   - Create the custom deployment folder if it doesn't exist.
   - Copy configuration files, collector files, the main configuration, and the executable to the deployment folder.
   - Create (or update) a Windows service named `SqlExporterSvc` that starts the deployed executable with the appropriate configuration.
3. Verify that the service is running and properly configured.

### Metrics and Testing

4. The exporter collects a variety of SQL Server metrics including:
   - **mssql_local_time_seconds:** Local server time in Unix seconds.
   - **mssql_connections:** Number of active connections.
   - **mssql_deadlocks:** Count of deadlock events.
   - **mssql_user_errors:** Count of user error events.
   - **mssql_kill_connection_errors:** Count of severe errors that result in killed connections.
   - **mssql_page_life_expectancy_seconds:** Seconds a page stays in the buffer pool.
   - **mssql_batch_requests:** Number of batch commands received.
   - **mssql_log_growths:** Number of log growth events per database.
   - **mssql_io_stall_seconds & mssql_io_stall_total_seconds:** I/O stall times for read and write operations.
   - **mssql_resident_memory_bytes & mssql_virtual_memory_bytes:** Memory usage metrics.
   - **mssql_memory_utilization_percentage:** Memory utilization percentage.
   - **mssql_page_fault_count:** Number of page faults.
   - **mssql_os_memory & mssql_os_page_file:** Operating system memory and page file usage.
   - **Waits:** Detailed wait statistics.
   - **auto_growth_last_day:** Auto-growth events recorded in the last day.
   - **DB Size Metrics:** Data, log, and total database sizes along with backup sizes.
   
   These metrics are exposed at the URL: `http://<exporter-host>:9399/metrics`.

5. To test the exporter, open a command prompt or browser and run:
   ```shell
   curl http://localhost:9399/metrics
   ```
   You should receive Prometheus-formatted metrics as output.

### Adding Exporter to PMM

6. To add this exporter as an external service in the PMM (Percona Monitoring and Management) tool:
   - Open the PMM UI in your browser.
   - Navigate to "Configuration" → "External Services" (or the section for adding external metrics).
   - Click "Add External Service" and enter the following details:
       • **Service Name:** Prometheus SQL Exporter  
       • **Metrics URL:** `http://<exporter-host>:9399/metrics`
   - Save your configuration. PMM will now scrape and display metrics from the SQL Exporter.
...
Adrien OBERNESSER @ dbi services extended the fork to package the SQL Server metrics for PMM as an external service with dashboard included. 
